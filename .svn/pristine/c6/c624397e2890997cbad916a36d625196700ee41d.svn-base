...
class KDTree
{
  ...
  private:
    ...
    // void buildTree(vector<Point<Dim>>& points, int left, int right, int k, int curDim);
    // void select(vector<Point<Dim>>& list, int left, int right, int k, int curDim);
    // int partition(vector<Point<Dim>>& list, int left, int right, int curDim);
    // *
    //  * @todo Add your helper functions here.
     
    void buildTree(int start, int end,  int curDim);
    void select(int start, int end, int dimension, int mid_index);
      int partition(int start, int end,  int curDim);
    Point<Dim> findNearestNeighborHelp(const Point<Dim>& query, int start, int end, int curDim) const;
    //  void findNearestHelper(const Point<Dim> & query, Point<Dim> & currentBest,
    //                                 int left, int right, int dimension, int minDis, bool & isFirst) const;
    int getDistance(const Point<Dim>& query, const Point<Dim>& p) const;
    vector< Point<Dim> > points;

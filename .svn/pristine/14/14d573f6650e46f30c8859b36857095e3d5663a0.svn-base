...
size_t searchSmall(const std::vector<T>& elements, int start, int end, const C& val)
{
    int n = (start + end)/2;
    if(elements[n] == val)
        return n;
    ...
    if(elements[n] < val){
        if(elements[n+1] > val)
            return (n+1);
        return searchSmall(elements, n+1, end, val);
    }
    else{
        if(elements[n-1] < val)
            return n;
        return searchSmall(elements, start, n-1, val);
    }
}
template <class T, class C>
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    if(val == elements[0])
        return 0;
    if(val > elements[size-1])
        return size;
    // for(unsigned long i = 0; i < size - 1; i++){
    //     if(val < elements[i + 1] || val == elements[i + 1])
    //         return i+1;
    // }
    return searchSmall(elements, 0, size-1, val);
}

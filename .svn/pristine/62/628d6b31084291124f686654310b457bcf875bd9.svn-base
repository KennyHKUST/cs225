...
int main(int argc, char *argv[])
{
    ...
    cout<<"testConstructor1"<<endl;
    //SkipList list (1337, RGBAPixel(137, 137, 137));

    RGBAPixel temp(10,11,12);
    SkipList test =SkipList(probability, maxLevelLog(img_size*img_size));
    int key = 3;
    ...
    RGBAPixel ret = test.search(7);
    if(ret != c)
        cout<<"sad"<<endl;

    if(ret == c)
        cout<<"great"<<endl;
    //RGBAPixel testfind = test->search(7);

    bool a = test.remove(3);
    bool b = test.remove(10);
    cout<< a << ' '<< b<< endl;

...
PNG brighten(PNG original, int amount)
{
    ...
    for (size_t yi = 0; yi < original.height(); yi++) {
	    ...
	    original(xi, yi)->red=original(xi, yi)->green+ amount;
	    original(xi, yi)->red=original(xi, yi)->blue+ amount;
        }
    }
    return original;
}
...
PNG blendImages(PNG firstImage, PNG secondImage)
{
    ...
    for (size_t yi = 0; yi < firstImage.height(); yi++) {
        for (size_t xi = 0; xi < firstImage.width(); xi++) {
            /// Your code here!
            firstImage(xi, yi)->red= 0.5*(firstImage(xi, yi)->red)+0.5*(secondImage(xi, yi)->red);
	    firstImage(xi, yi)->green= 0.5*(firstImage(xi, yi)->green)+0.5*(secondImage(xi, yi)->green);
  	    firstImage(xi, yi)->blue= 0.5*(firstImage(xi, yi)->blue)+0.5*(secondImage(xi, yi)->blue);
	}
    }
    return firstImage;
}

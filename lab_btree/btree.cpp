...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    //cout<<"42"<<endl;
    //cout<<"first_larger_idx is "<<first_larger_idx<<endl;
    //cout<<"subroot->elements[first_larger_idx]: "<<subroot->elements[first_larger_idx].key<<endl;
    if(first_larger_idx >= subroot->elements.size()){//prevent undefined accessment
        if(subroot->is_leaf)
            return V();
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
   ...
   // cout<<*child<<endl;
    ...
    //cout<<"170"<<endl;
    /* TODO Your code goes here! */
    //assign the elements into the new_right
    new_right->elements.assign(mid_elem_itr+1, child->elements.end());
    //not sure if checking whether child exsits is needed
    new_right->children.assign(mid_child_itr, child->children.end());
    //insert the right child into the parent node children vector
    parent->children.insert(child_itr, new_right);
    //insert the mid element of the child into the parent node elemet vector
    parent->elements.insert(elem_itr, *mid_elem_itr);
    //do the same as the new_right
    //cout<<"180"<<endl;
    ...
    //cout<<"new_left is "<<*new_left<<endl;
    //cout<<"new_right is "<<*new_right<<endl;
}
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
     ...
     */
    //cout<<"recursive insert function"<<endl;
    if(subroot->is_leaf){
        BTreeNode* node = new BTreeNode(true, order);
    ...
    for(unsigned long i = 0; i < subroot->children.size(); i++){

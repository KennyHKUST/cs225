...
typename List<T>::ListNode* List<T>::merge(ListNode* first, ListNode* second)
{
    ...
    ListNode* temp;
    if(p1->next == NULL && p2->next == NULL)//both have one datapoint only
        if(p1->data < p2->data){
            p1->next = p2;
            p2->prev = p1;
            p2->next = NULL;
    ...
    while(p1 != NULL && p2 != NULL){
            ...
            while(p1->data < p2->data && p1->next != NULL)
                p1 = p1->next;
            temp = p2->next;
            if (p1 == first && p2->data < p1->data){//if p2 < p1 when p1 == head
                if(p2->next != NULL){
            ...
            if (p1->next == NULL && p1->data < p2->data){//if p1 < p2 when p1 == tail
                p1->next = p2;
                p2->prev = p1;
                break;
            }
            if(p1->prev != NULL){//link the memory in normal case
                p1->prev->next = p2;
            }
            else
                first = p2;//if p1 has only one data point and p2 == p1
            p2->prev = p1->prev;
            p2->next = p1;
            p1->prev = p2;
            p2 = temp;
...
typename List<T>::ListNode* List<T>::mergesort(ListNode* start, int chainLength)
{
    ...
    int splitpoint;
    if(chainLength % 2 != 0)
        splitpoint = chainLength/2 + 1;
    else
        splitpoint = chainLength/2;
    ListNode* start2 = split(start, splitpoint);
    cout<<"start->data = "<<start->data<<endl;
    cout<<"start2->data = "<<start2->data<<endl;
    ListNode* p2 = mergesort(start2, chainLength - splitpoint);
    ListNode* p1 = mergesort(start, splitpoint);

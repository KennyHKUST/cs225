                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if(!mergeQueue.empty() && !singleQueue.empty()){
      if(singleQueue.front()->freq.getFrequency() >
      ...
      singleQueue.pop();
    //cout<<smallest->freq.getFrequency()<<endl;;
    //cout<<smallest<<endl;
    return smallest;
}
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    TreeNode* l1 = NULL;
    TreeNode* l2 = NULL;
    //cout<<mergeQueue.size()<<endl;
    //cout<<singleQueue.front()->freq.getFrequency()<<endl;
    while(true){
      //cout<<"???"<<endl;
      //cout<<"???"<<endl;
      l1 = removeSmallest(singleQueue, mergeQueue);
      l2 = removeSmallest(singleQueue, mergeQueue);
      //cout<<"aha"<<endl;
      //cout<<l1->freq.getFrequency()<<endl;
      cout << "l1 = "<<l1<<endl;
      cout << "l2 = "<<l2<<endl;
      TreeNode* i = new TreeNode(l1->freq.getFrequency() + l2->freq.getFrequency());
      i->left = l1;
      i->right = l2;
      mergeQueue.push(i);
      if((singleQueue.empty() && mergeQueue.size() == 1)|| (singleQueue.size() == 1 && mergeQueue.empty()))
      	break;
    }

    if(!mergeQueue.empty()){
      mergeQueue.front()->left = l1;
      mergeQueue.front()->right = l2;
      root = mergeQueue.front();
    }
    else{
      cout<<"aaa"<<endl;
      singleQueue.front()->left = l1;
      singleQueue.front()->right = l2;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(bfile.getNextBit() == 1){
      TreeNode* l = new TreeNode(Frequency(bfile.getNextByte(), 0));
      return l;
    }

    TreeNode* i = new TreeNode(0);
    i->left = readTree(bfile);
    i->right = readTree(bfile);
    return i;
}

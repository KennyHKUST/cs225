...
void SkipList::insert(int key, RGBAPixel value)
{
    ...
    cout<<"Forward key = "<<traverse->nodePointers[0].next->key<<endl;
    int forwardLevel = 0;
    int backwardLevel = 0;
    ...
    {
        cout<<"ForwardLevel = "<<forwardLevel<<endl;
        cout<<"MaxLevel = "<<maxLevel<<endl;
        if(forward == tail)
        {
            cout<<"forward == tail"<<endl;
            if(forward->nodePointers[forwardLevel].prev == NULL)
                forward = forward->nodePointers[forwardLevel-1].prev;

            cout<<"backward key = "<<forward->nodePointers[forwardLevel].prev->key<<endl;
            cout<<"Forward key = "<<forward->key<<endl;
        ...
        if(forward->nodePointers.size()-1 > (size_t)forwardLevel)//error
        {
            if(forward->nodePointers.size() > (size_t)newNodeLevel)
                break;
            cout<<"nodePointers.size()>forwardLevel"<<endl;
            cout<<"forward->nodePointers.size()-1 = "<<forward->nodePointers.size()-1<<endl;
            cout<<"forward->nodePointers[forwardLevel]->prev->nodePointers.size()-1 = "<<forward->nodePointers[forwardLevel].prev->nodePointers.size()-1<<endl;
            cout<<"forward level = "<<forwardLevel<<endl;
            if(forward->nodePointers[forwardLevel].prev == NULL)
                forward = forward->nodePointers[forwardLevel-1].prev;
            cout<<"backward key = "<<forward->nodePointers[forwardLevel].prev->key<<endl;
            cout<<"Forward key = "<<forward->key<<endl;
        ...
        {
            cout<<"enter into else"<<endl;
            forward = newNode;
            forward = forward->nodePointers[0].next;
            cout<<"forward is now = "<<forward->key<<endl;
        }
    }
    cout<<"At line 135"<<endl;
    while (backwardLevel < maxLevel)
    {
        cout<<"BackwardLevel = "<<backwardLevel<<endl;
        cout<<"MaxLevel = "<<maxLevel<<endl;
        if(prev == head)
        {
            cout<<"traverse == head"<<endl;
            prev->nodePointers[backwardLevel].next = newNode;
            newNode->nodePointers[backwardLevel].prev = prev;
            backwardLevel++;
        }

        if(prev->nodePointers.size()-1 > (size_t)backwardLevel)
        {
            cout<<"nodePointers.size()>backwardlevel"<<endl;
            prev->nodePointers[backwardLevel].next = newNode;
            newNode->nodePointers[backwardLevel].prev = prev;
            backwardLevel++;
        }
        ...
        else
        {
            ...
            cout<<"prev is now = "<<prev->key<<endl;
        }
    }

    cout<<"insert function end"<<endl;
}
/**

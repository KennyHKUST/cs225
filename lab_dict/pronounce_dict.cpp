...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    /* Your code goes here! */
    string firstw = word1;
    string secondw = word2;
    std::transform(firstw.begin(), firstw.end(), firstw.begin(), toupper);
    std::transform(secondw.begin(), secondw.end(), secondw.begin(), toupper);
    auto pro1 = dict.find(firstw);
    auto pro2 = dict.find(secondw);

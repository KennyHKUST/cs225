...
void testGradientPicker()
{
    ...
    testColorPicker(gradientPicker)
        .writeToFile("images/gradientColorPickerTest.png");
...
void testSolidFill(const Filler& filler, const string& outputFile)
{
    ...
    animation anim;
    if (filler == DFS)
        anim = filler::dfs::fillSolid(img, SOLIDX, SOLIDY, px, SOLIDTOLERANCE,
                                      SOLIDFRAMEFREQ);
    else
        anim = filler::bfs::fillSolid(img, SOLIDX, SOLIDY, px, SOLIDTOLERANCE,
                                      SOLIDFRAMEFREQ);
...
void testGridFill(const Filler& filler, const string& outputFile)
{
    ...
    animation anim;
    if (filler == DFS)
        anim = filler::dfs::fillGrid(img, GRIDX, GRIDY, px, GRIDGRIDSPACING,
                                     GRIDTOLERANCE, GRIDFRAMEFREQ);
    else
        anim = filler::bfs::fillGrid(img, GRIDX, GRIDY, px, GRIDGRIDSPACING,
                                     GRIDTOLERANCE, GRIDFRAMEFREQ);
    anim.write(outputFile);
    cout << "\tWrote " << outputFile << endl;
}
...
void testGradientFill(const Filler& filler, const string& outputFile)
{
    ...
    animation anim;
    if (filler == DFS)
        anim = filler::dfs::fillGradient(img, GRADIENTX, GRADIENTY, px, px2,
                                         GRADIENTRADIUS, GRADIENTTOLERANCE,
                                         GRADIENTFRAMEFREQ);
    else
        anim = filler::bfs::fillGradient(img, GRADIENTX, GRADIENTY, px, px2,
                                         GRADIENTRADIUS, GRADIENTTOLERANCE,
                                         GRADIENTFRAMEFREQ);
    anim.write(outputFile);
    cout << "\tWrote " << outputFile << endl;
...
void pacmanTests()
{
    ...
    animation pacManBFS = filler::bfs::fill(
        img, img.width() / 2, img.height() / 2, BFSfiller, 8000, INT_MAX);
    ...
    animation pacManDFS = filler::dfs::fill(
        img, img.width() / 2, img.height() / 2, DFSfiller, 8000, INT_MAX);
    ...
    cout<<"reached line 219"<<endl;
    filler::bfs::fillSolid(antiImg, 10, 10, grey, 8000, INT_MAX);
    cout<<"reached line 221"<<endl;
    filler::bfs::fillSolid(antiImg, antiImg.width() / 2, antiImg.height() / 2,
                           black, 8000, INT_MAX);
    cout<<"reached line 224"<<endl;
    // ANTI PAC MAN BFS
    img = antiImg;
...
int main(int argc, char** argv)
{
    ...
    // Testing the fills themselves.  Note that this uses the functors, so make
    // sure those are working first
    if (solid) {

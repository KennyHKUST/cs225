...
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test)
{
    ...
    if(word.length() != test.length())
        return false;
    Dict<char, int> hashTableWord(word.length());
    Dict<char, int> hashTableTest(word.length());
    ...
    for(unsigned long i = 0; i < word.length(); i++){
        ...
        //std::cout<<hashTableTest.find(word[i])<<std::endl;
        if(hashTableWord.find(word[i]) != hashTableTest.find(word[i]))
            return false; 
    }
    return true;
}
